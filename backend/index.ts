  ...
  'GET /api/search': [
    async ({ query }) => {
      ...
      try {
        ...
        const detailsData = (await detailsResponse.json()) as { items?: VideoItem[] };
        ...
        const songs = (searchData.items ?? [])
          .map(item => {
            ...
            if (!id || duration < 75 || /#shorts|\bshorts\b/i.test(title)) return null;
        ...
        console.error('Music search failed', cause instanceof Error ? cause.message : 'Unknown error');
